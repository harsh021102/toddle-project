...
import { toddlelogo,search,bookmark,onlylogo,back, bookmarkblack} from '../misc/ImagesAndIcons'
import CreateButton from './CreateButton'
import { useLocation, useNavigate } from 'react-router-dom';

const NavBar = ({toggleCreateBoardCard,showcard,title}) => {
  ...
  if(path==="/posts")
  {
    return (
      <div className='flex justify-between items-center py-[12px] px-[48px] absolute w-full h-[72px] left-0 top-0 border-b-[1px] border-[#EBEBEB] bg-white'>
          <div className="flex gap-2 justify-between items-center min-w-[270px] h-[30px]">
            <button onClick={handleClick}>
              <img src={back} alt="back logo"/>
            </button>
            <img src={onlylogo} alt="toddle logo"/>
            <h1 className='font-avenir-bold font-[700] text-[18px] leading-[24px] text-[#222222] w-[290px] '>{`${title}`}</h1>
          </div>
          ...
          W-[89px] h-[20px] gap-[24px]'>
            ...
            <button className='w-[20px] h-[20px]' onClick={()=>{navigate('/bookmark')}}>
  ...
  else if(path==="/bookmark")
  {
    return (
      <div className='flex justify-between items-center py-[12px] px-[48px] absolute w-full h-[72px] left-0 top-0 border-b-[1px] border-[#EBEBEB] bg-white'>
          <div className="flex justify-between items-center min-w-[200px] h-[20px]">
            <button onClick={handleClick}>
              <img src={back} alt="back logo"/>
            </button>
            <img src={onlylogo} alt="toddle logo"/>
            <h1 className='font-avenir-bold font-[700] text-[18px] leading-[24px text-[#222222]'>My bookmarks</h1>
          </div>
          <div className={showsearch?`flex justify-between items-center h-[40px]  w-[506px]`:`flex justify-end items-center h-[40px]  w-[506px]`}>
          <div className={showsearch?`flex justify-start items-center h-[40px] border-[1px] border-[#EBEBEB] rounded-[6px] gap-[24px] py-[13px] px-[16px]`:'hidden'}>
            <img src={search} alt="search icon" className='text-[#B0B0B0]'/>
          ...
          W-[89px] h-[20px] gap-[24px]'>
            ...
            <button className='w-[20px] h-[20px]'>
              <img src={bookmarkblack} alt="bookmark icon" className='text-[#222222]'/>
            </button>
          </div>
          </div>
      </div>
